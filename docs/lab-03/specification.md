...
- BR-15: Status transitions follow the table below. Operational updates require expected version, increment version and update updatedAt atomically. Terminal tickets allow reads and Public Comments/Notes, but reject assignment/priority changes until reopened where permitted.
- BR-16: Problem Appears Resolved records an owner indication with backend time for OPEN, IN_PROGRESS, WAITING_FOR_REQUESTER or REOPENED only; it does not change status. Repeating it is idempotent. Reopening clears the indication.
