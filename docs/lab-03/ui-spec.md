...
- Comments/Notes: visibly separate headings, forms and explanations; Internal Notes clearly labeled Staff/Admin only. No shared visibility toggle, edit or delete. Show server-backed 20-entry pages with Previous/Next and total count, including histories beyond the first page. After posting, load the last page so the new entry remains visible after refresh. Disable posting duplicates while busy; preserve text on errors, clear only after confirmed success. Render submitted content as text.
